...
class TradeRecord(Base):
    ...
    def __repr__(self):
        return "<TradeRecord( account_id='%s',symbol='%s',name='%s',side='%s', price='%s',volume='%s', amount='%s',strategy='%s',trade_date='%s',trade_time='%s')>" % (
    ...
    def batch_insert(self,df):
        for i,execrpt in df.iterrows():
            if execrpt.exec_type == 15:
                ...
                trade_record.mode = execrpt.mode
                ...
                if TradeRecord().hasExistTradeRecord(trade_record.symbol, trade_record.trade_time,
...
if __name__ == '__main__':
    ...
    # record.insert()
    print(TradeRecord().getAccountId())
    df= TradeRecord().hasExistTradeRecord('SHSE.600743','2022-05-17 13:52:07.693056',33120)
