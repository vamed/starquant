...
def cpc_index(returns, prepare_returns=True):
    ...
    (profit factors * win % * win loss ratio)
    """
    if prepare_returns:
        returns = _utils._prepare_returns(returns)
...
def common_sense_ratio(returns, prepare_returns=True):
    """Measures the common sense ratio (profit factors * tail ratio)"""
    if prepare_returns:
        returns = _utils._prepare_returns(returns)
...
def recovery_factor(returns, prepare_returns=True):
    """Measures how fast the strategys recovers from drawdowns"""
    if prepare_returns:
        returns = _utils._prepare_returns(returns)
...
def kelly_criterion(returns, prepare_returns=True):
    ...
    should be allocated to the given strategys, based on the
