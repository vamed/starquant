...
def full(returns, benchmark=None, rf=0., grayscale=False,
    ...
    if _utils._in_notebook():
        ...
        metrics(returns=returns, benchmark=benchmark,
                ...
                prepare_returns=False)
        loginfo('\n\n')
        loginfo('[5 Worst Drawdowns]\n')
        if dd_info.empty:
            loginfo("(no drawdowns)")
        else:
            loginfo(_tabulate(dd_info, headers="keys",
                            tablefmt='simple', floatfmt=".2f"))
        loginfo('\n\n')
        loginfo('[Strategy Visualization]\nvia Matplotlib')
...
def metrics(returns, benchmark=None, rf=0., display=True,
    ...
    if display:
        loginfo(_tabulate(metrics, headers="keys", tablefmt='simple'))
        return None
